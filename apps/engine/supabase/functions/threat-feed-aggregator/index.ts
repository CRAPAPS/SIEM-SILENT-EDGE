...
async function fetchThreatFox(): Promise<SyncResult> {
  const t0 = Date.now();
  const proxySecret = Deno.env.get("THREAT_FEED_PROXY_SECRET");
  if (!proxySecret) {
    return { source: "threatfox", iocs_added: 0, iocs_updated: 0, error: "THREAT_FEED_PROXY_SECRET not set", duration_ms: 0 };
  }

  try {
    const res = await fetch("https://console.shelinfosec.com/api/threat-feed/threatfox", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    if (!res.ok) {
      ...
      return { source: "threatfox", iocs_added: 0, iocs_updated: 0, error: "Proxy HTTP " + res.status + " " + text.slice(0, 100), duration_ms: Date.now() - t0 };
    }

    const data = await res.json() as { iocs_added?: number; error?: string; duration_ms?: number };
    if (data.error) {
      return { source: "threatfox", iocs_added: 0, iocs_updated: 0, error: data.error, duration_ms: Date.now() - t0 };
    }

    return { source: "threatfox", iocs_added: data.iocs_added ?? 0, iocs_updated: 0, duration_ms: Date.now() - t0 };
  } catch (err) {
